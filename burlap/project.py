...
def init_dj(project_name, default_roles, virtualenv_dir='.env', version=None, **kwargs):
    ...
    if version:
        os.system(f'{virtualenv_dir}/bin/pip install Django=={version}')
    else:
    ...
    if not os.path.isdir('src/%s' % site_name):
        ...
        with open(_settings_fn, encoding='utf8') as fin:
            _content = fin.read()
...
'''.format(
        ...
        for _role in default_roles:
            _top.append(f"ROLE_{_role.upper()} = '{_role.lower()}'")
        _top.append('ROLES = (')
        for _role in default_roles:
            _top.append(f"    ROLE_{_role.upper()},")
        _top.append(')')
        _index = _content.find('"""\n\n') + 4

        bottom_args = dict(app_name=project_name, app_name_title=project_name.title() + ' Administration', app_name_simple=project_name.title())
        _bottom = '''
...
'''.format(**bottom_args)
        with open(_settings_fn, 'w', encoding='utf8') as fout:
            fout.write(_content[:_index] + _sites + ('\n'.join(_top)) + _content[_index:] + _bottom)

    print('Creating Django helper scripts...')
    open('src/manage', 'w', encoding='utf8').write(
        '''#!/bin/bash
...
../.env/bin/python manage.py $@'''
    ...
    open('src/runserver', 'w', encoding='utf8').write(
        '''#!/bin/bash
...
./manage runserver localhost:$PORT'''
    ...
    open('src/shell', 'w', encoding='utf8').write(
        r'''#!/bin/bash
...
PS1="\u@\h:\W(fab)\$ "'''
    ...
    if not os.path.isdir('src/%s' % project_name):
        os.system(f'cd src; ./manage startapp {project_name}')
...
class ProjectSatchel(ContainerSatchel):
    ...
    def update_settings(self, d, role, path='roles/{role}/settings.yaml'):
        ...
        data = load_func(open(settings_fn, encoding='utf8'), Loader=yaml.SafeLoader)
        data.update(d)
        settings_str = dump_func(data)
        open(settings_fn, 'w', encoding='utf8').write(settings_str)
    ...
    def create_skeleton(self, project_name, roles='', components='', pip_requirements='', virtualenv_dir='.env', **kwargs):
        ...
        open('roles/all/settings.yaml', 'w', encoding='utf8').write(
            self.render_to_string('burlap/all_settings.yaml.template', extra=dict(project_name=project_name, site_name=site_name, app_name=app_name))
        )
        for _role in default_roles:
            open('roles/%s/settings.yaml' % _role, 'w', encoding='utf8').write(
                self.render_to_string('burlap/role_settings.yaml.template', extra=dict(project_name=project_name, site_name=site_name, role=_role))
            )

        default_packages = pip_requirements.split(',') if pip_requirements else []
        if default_packages:
            open('roles/all/pip-requirements.txt', 'w', encoding='utf8').write('\n'.join(default_packages))

        print('Adding global apt-requirements.txt...')
        open('roles/all/apt-requirements.txt', 'w', encoding='utf8').write('')

        print('Adding fabfile...')
        content = open(fabfile_template, encoding='utf8').read()
        ...
        open('.gitignore', 'w', encoding='utf8').write(self.render_to_string('burlap/gitignore.template'))
        ...
        print('=' * 80)
        print()
        print(f'Skeleton created for project {project_name.title()}!')
        print()
    ...
    def add_roles(self, roles):
        for role in roles:
            ...
            if not os.path.isdir(fn_dir):
                os.makedirs(fn_dir)
            open(fn, 'w', encoding='utf8').write(self.render_to_string('burlap/role_settings.yaml.template', extra=dict(role=_role)))
            print('Added role %s!' % role)

    @task
    def create_satchel(self, name):
        name_simple = re.sub(r'[^a-z0-9]+', '', name.lower())
        content = self.render_to_string('burlap/satchel.py.template', extra=dict(
